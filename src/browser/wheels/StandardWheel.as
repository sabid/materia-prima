package browser.wheels
{
	...
	public class StandardWheel extends Wheel
	{
		...
		public function countFiles():void
		{
			...
			// adding linked folders to folder content
			...
			// adding control elements (from foldername.xml) to contents
		...
		// Calls loadNextItem() for the first time
		...
		// Searches contents for the given item and displays it if it exists
		...
		// Decides how to display the given item.
		// If contents[j] is a known filetype, an icon of the right type will be displayed, 
		// otherwise 1 will be added to j and loadNextItem() will be called again
		protected function displayItem(objectIn):void
		{
			...
			else if(contents[j].isDirectory)
			{
				...
				// (What the fuck??? hat das irgendeinen sinn hier? Das sollte ich nochmal prüfen, wenn ich wieder nüchtern bin)
				var wheelType:String = type;
				
				var links = infoXML.LINKS;
				
				// If links contains contents[j], that means it is a link
				// In that case wheelType will be set to the type found in links. Same with the icon.
				...
				// If contents[j] contains a swf-file
